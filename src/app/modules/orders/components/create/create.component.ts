...
export class CreateOrderComponent implements OnInit {
  ...
  initializeLocalStorage() {
    const orderproducts =
      this.persistanceService.get('orderdata') || null;
        ...
        tableRows: this.fb.array(
          orderproducts.map((item) =>
            this.fb.group({
              ...
              trade_price: [
                { value: item.trade_price, disabled: true }
              ],
              ...
              summa: [
                { value: item.price * item.quantity, disabled: true }
              ],
              description: ''
            })
          )
  ...
  orderDispatch() {
    ...
    this.createOrder.ordertocm =
      1 * this.formPayed.controls.ordertocm.value;
  ...
  correctTotal() {
    if (this.formTable) {
      this.createOrder.total = this.formTable.value.tableRows.reduce(
  ...
  onInputPercent(index: number) {
    const percent = (<FormArray>(
      this.formTable.get('tableRows')
    )).controls[index].get('percent').value;
    const quantity = (<FormArray>(
      this.formTable.get('tableRows')
    )).controls[index].get('quantity').value;
    const tradePrice = (<FormArray>(
      this.formTable.get('tableRows')
    )).controls[index].get('trade_price').value;

    if (percent && quantity && tradePrice) {
      (<FormArray>this.formTable.get('tableRows')).controls[index]
        .get('price')
        .setValue(percent * tradePrice);
      (<FormArray>this.formTable.get('tableRows')).controls[index]
        .get('summa')
        .setValue(quantity * percent * tradePrice);
      this.correctTotal();
    }
  }
  ...
  onInputPrice(index: number) {
    ...
    const price = (<FormArray>(
      this.formTable.get('tableRows')
    )).controls[index].get('price').value;

    if (price && quantity && price) {
      (<FormArray>this.formTable.get('tableRows')).controls[index]
        .get('percent')
        .setValue(price / tradePrice);
      (<FormArray>this.formTable.get('tableRows')).controls[index]
        .get('summa')
  ...
  saveToLocalStorage() {
    this.persistanceService.set(
      'orderdata',
      (<FormArray>this.formTable.get('tableRows')).controls.map(
        (item) => ({
  ...
  onClient($event: ClientInterface) {
    this.formClient = this.fb.group({
      ...
      fullname: [
        { value: $event.fullname, disabled: true },
        Validators.required
      ],
      phone: [
        { value: $event.phone, disabled: true },
        Validators.required
      ]
    });
