...
import { salesAction, cassaAction } from '../../store/actions/action';
import {
  isLoadingSelector,
...
export class SalesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.salesSubscription = this.store
      .pipe(select(currentSalesSelector), filter(Boolean))
      .subscribe((currentSales: SalesInterface[]) => {
        this.sales = currentSales;
        this.sum = this.sales.reduce(
          (sum, current) => sum + current.price * current.quantity,
          0
        );
      });

    this.returnSalesSubscription = this.store
      .pipe(select(isReturnSalesSelector), filter(Boolean))
      .subscribe((returnSales: ReturnSalesInterface[]) => {
        this.returnSum = returnSales.reduce(
          (sum, current) =>
