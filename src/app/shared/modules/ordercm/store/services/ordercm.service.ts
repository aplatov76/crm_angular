...
export class OrderCmService {
  ...
  getOrders(query?: any): Observable<OrderCmInterface[]> {
    ...
    if (query) {
      if (query.databegin && query.dataend)
        queryRequest = `databegin=${query.databegin}&dataend=${query.dataend} 23:59`;
    }

    return this.http.get<OrderCmInterface[]>(
      `${environment.url}/cm/orders/?${queryRequest}`
    );
  }

  getOrder(id: number): Observable<OrderCmInterface> {
    return this.http.get<OrderCmInterface>(
      `${environment.url}/cm/orders/${id}`
    );
  }

  setOrderData(
    orderdata: OrderCmDataInterface[]
  ): Observable<OrderCmDataInterface[]> {
    // console.log(orderdata)
    return this.http.post<OrderCmDataInterface[]>(
      `${environment.url}/cm/orders/`,
      { orderdata }
    );
  }
  ...
  sendOrderToCM(orderdata: OrderCmDataInterface[]): Observable<any> {
    return this.http.post<any>(
      `${environment.url}/cm/orders/mailer`,
      { orderdata }
    );
  }
