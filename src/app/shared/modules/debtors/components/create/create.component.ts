...
export class CreateDebtorComponent implements OnInit {
  ...
  initializeForm(product: ProductInterface) {
    this.form = this.fb.group({
      ...
      stock: [
        { value: product.stock, disabled: true },
        [Validators.required]
      ],
      price: [
  ...
  deletItem(id: number): void {
    ...
    this.currentDebtorData = this.currentDebtorData.filter(
      (item) => item.id !== id
    );
  }
  ...
  debtorDispatch() {
    const createDebtor: any = {
      ...
      debtordata: this.currentDebtorData.map((item) => ({
        id: item.product.id,
  ...
  submit() {
    ...
    const searchDublicate =
      this.currentDebtorData.find(
        (res) => res.product.id === tmp.id.value
      ) || null;
    ...
    if (this.form.controls.quantity.invalid) {
      this.toastr.error(
        `"${tmp.title.value}" недостаточно на складе!`
      );
      return;
    }
    ...
    this.currentDebtorData.push({
      id: 0,
      product: { id: tmp.id.value, title: tmp.title.value },
      quantity: tmp.quantity.value,
