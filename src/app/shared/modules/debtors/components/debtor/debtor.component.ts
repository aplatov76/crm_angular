        ...
        let sum = 0;
         ...
         let docDefinition = {
             ...
             content: [
                 ...
                 {text: `Договор № ${this.id} от ${this.datepipe.transform(order.data, 'yyyy-MM-dd')}`, style: 'header' },
                 ...
                 {text: `Перечь товаров:`, style: 'h6' },
                 {
                     ...
                     table: {
                         ...
                         body: [
                             ...
                             ...order.debtorsdata.map((item, index) => [index+1, item.product.title, item.quantity, item.price.toFixed(2), (item.quantity*item.price).toFixed(2)])
        ...
        let sum = 0;
         ...
         let docDefinition = {
             content: [
                 ...
                 {
                     text: `Договор купли-продажи с рассрочкой платежа № ${this.id} от ${this.datepipe.transform(order.data, 'yyyy-MM-dd')}`,
                     style: 'header'
                },
                {
                ...
                {
                     text: `Индивидуальный предприниматель Платов Ю.В. ИНН290900038458 государственный регистрационный номер 304290529200029, именуемый далее Продавец, с одной стороны и ${order.client.fullname}, паспорт: ${order.client.passport_number} выдан ${this.datepipe.transform(order.client.passport_data, 'yyyy-MM-dd')} , зарегистрированный(ая) по адресу : ${order.client.register_address} именуемый(ая) далее Покупатель с другой стороны , заключили настоящий договор о нижеследующем.`, 
                     style: 'text'
                },
                {
                    text: `Покупатель приобрел товар согласно перечня в товарном чеке №: ${order.id} от ${this.datepipe.transform(order.data, 'yyyy-MM-dd')} с рассрочкой платежа на срок до: ${this.datepipe.transform(endData, 'yyyy-MM-dd')} по розничной цене на сумму ${order.total} руб.`,
                    style: 'text'
                },
                {
                ...
                {
                    text: `Остаточную сумму: ${order.current} руб., Покупатель обязуется оплатить Продавцу, предоставившему рассрочку платежа для покупки товаров , до ${this.datepipe.transform(endData, 'yyyy-MM-dd')}. Остаточную сумму Покупатель обязуется внести наличными деньгами в кассу Продавца, согласно графика погашения задолженности не позднее чем ${this.datepipe.transform(endData, 'yyyy-MM-dd')}.`,
                    style: 'text'
                },
                {
                ...
                {
                    text: `Покупатель: ________________, ${order.client.fullname}, дата: ${this.datepipe.transform(order.data, 'yyyy-MM-dd')} г.`,
                    style: 'text'
                },
                {
                    text: `Продавец : __________________, Платов Юрий Витальевич, дата:  ${this.datepipe.transform(order.data, 'yyyy-MM-dd')} г.`,
                    style: 'text'
                },
