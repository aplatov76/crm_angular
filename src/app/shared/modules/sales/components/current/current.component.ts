...
import { addSaleAction } from '../../store/actions/action';
import {
  currentError,
  currentSaleCompleted
} from '../../store/selectors';
import {
  currentDataSelector as currentPraisSelector,
  currentProductSelector
} from '../../../../utilmodules/prais/store/selectors';
import { CurrentSaleInterface } from '../../interfaces/currentSale.interface';
import { PraisInterface } from '../../../../interfaces/prais.interface';
import {
  praisAction,
  productAction
} from '../../../../utilmodules/prais/store/actions/action';
...
export class CurrentSaleComponent {
  ...
  initializeListeners(): void {
    ...
    this.isCurrentProduct$ = this.store
      .pipe(select(currentProductSelector), filter(Boolean))
      .subscribe((res: ProductInterface[]) => {
        if (res) {
          [this.isCurrentProduct] = res;
          this.initializeForm();
        }
      });
    ...
    this.errorSubscription = this.store
      ...
      .subscribe((res: any) =>
        this.toastr.error(`Произошла ошибка ${res.error.message}`)
      );

    this.subCompleted = this.store
      .pipe(select(currentSaleCompleted), filter(Boolean))
  ...
  initializeForm() {
    this.form = this.fb.group({
      ...
      stock: [
        { value: this.isCurrentProduct.stock, disabled: true },
        [Validators.required]
      ],
      price: [
        { value: this.isCurrentProduct.price, disabled: true },
        [Validators.required]
      ],
      quantity: [
        1,
        [
          Validators.required,
          Validators.min(1),
          Validators.max(this.isCurrentProduct.stock)
        ]
      ],
      sum: [
        { value: this.isCurrentProduct.price, disabled: true },
  ...
  initializeDeliveryForm(client: ClientInterface) {
    this.deliveryForm = this.fb.group({
      residence_address: [
        { value: client.residence_address, disabled: true },
        [Validators.required]
      ],
      clientid: [client.id, Validators.required],
      phone: [
        { value: client.phone, disabled: true },
        [Validators.required]
      ],
  ...
  submit() {
    const searchDublicate =
      this.currentSale.find(
        (res) => res.id === this.isCurrentProduct.id
      ) || null;

    if (searchDublicate) {
      this.toastr.warning(
        `"${this.isCurrentProduct.title}" уже есть в списке`
      );
    }
    if (this.form.controls.quantity.invalid) {
      this.toastr.error(
        `"${this.isCurrentProduct.title}" недостаточно на складе!`
      );
    }
    ...
    if (!this.form.invalid && searchDublicate === null) {
      ...
      this.currentSum +=
        this.isCurrentProduct.price *
        this.form.controls.quantity.value;
      this.toastr.success(
        `"${this.isCurrentProduct.title}" добавлено в текущую продажу`
      );
    }
  }
  ...
  saleDispatch() {
    this.store.dispatch(
      addSaleAction({
        ...
        delivery:
          this.delivery && this.deliveryForm
  ...
  validateDelivery(): boolean {
    if (this.delivery) {
      return !!(!this.deliveryForm || this.deliveryForm.invalid);
    }
    return false;
  }
  ...
  createCheck(createdSales: SalesInterface[]) {
    const docDefinition = {
      ...
      content: [
        {
          text: `Товарный чек № ${
            createdSales[0].check.id
          } от ${this.datepipe.transform(
            createdSales[0].check.data,
            'yyyy-MM-dd'
          )}`,
          style: 'header'
        },
