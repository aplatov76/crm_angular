...
export class SalesService {
  ...
  getSales(
    databegin: string = new Date().toISOString().slice(0, 10),
    dataend: string = new Date().toISOString().slice(0, 10),
    productId?: number,
    order?: string
  ): Observable<SalesInterface[]> {
    ...
    return this.http.get<SalesInterface[]>(
      `${environment.url}/sales?${query}`
    );
  }
  ...
  getCheck(checkid: number): Observable<SalesInterface[]> {
    ...
    if (checkid) query += `&checkid=${checkid}`;
  ...
  getPrais(): Observable<PraisInterface[]> {
    return this.http.get<PraisInterface[]>(
      `${environment.url}/product?type=product`
    );
  }

  setSale(
    currentSules: CurrentSaleInterface[],
    delivery: CurrentDelivery
  ): Observable<any> {
    const serviceCurrentSules = currentSules.map((el) => ({
      id: el.id,
      quantity: el.quantity
    }));

    return this.http.post<any>(`${environment.url}/sales`, {
      sale: serviceCurrentSules,
      delivery
    });
  }
  ...
  ): Observable<CassaValueInterface> {
    return this.http.get<CassaValueInterface>(
      `${environment.url}/cassa?data=${data}`
    );
  }
  ...
  getOrdersPayed(
    ...
    return this.http.get<OrderInterface[]>(
      `${environment.url}/order/payed?databegin=${databegin}&dataend=${dataend} 23:59`
    );
  }
  ...
  getDebtorPayed(
    ...
    return this.http.get<DebtorInterface[]>(
      `${environment.url}/debtors/payed?databegin=${databegin}&dataend=${dataend} 23:59`
    );
  }

  setCassaValue(sum: number): Observable<CassaValueInterface> {
    return this.http.post<CassaValueInterface>(
      `${environment.url}/cassa`,
      { cassa: { sum } }
    );
  }
}
